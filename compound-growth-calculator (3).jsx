...
const CompoundGrowthCalculator = () => {
  ...
  const InfoTooltip = ({ tooltipKey }) => {
    ...
    return (
      <div className="relative inline-block ml-1.5 md:ml-2 tooltip-container">
        <button
        ...
        {isActive && tooltip && (
          <div className="fixed sm:absolute w-[calc(100vw-2rem)] sm:w-72 md:w-80 bg-slate-900 border border-emerald-500/30 rounded-lg p-3 md:p-4 shadow-2xl left-4 sm:left-6 right-4 sm:right-auto top-auto sm:top-[-8px]" style={{ zIndex: 9999 }}>
            <div className="flex justify-between items-start mb-2">
              <h4 className="text-emerald-400 font-semibold text-sm">{tooltip.title}</h4>
              <button onClick={() => setActiveTooltip(null)} className="sm:hidden text-slate-400 hover:text-white p-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
            <p className="text-slate-300 text-xs mb-2 md:mb-3">{tooltip.description}</p>
            {tooltip.formula && (
              <div className="bg-slate-800/50 rounded px-2 py-1 mb-2 md:mb-3 overflow-x-auto">
                <code className="text-amber-400 text-xs whitespace-nowrap">{tooltip.formula}</code>
              </div>
            )}
  ...
  const StrategyCard = ({ strategyKey, strategy, isSelected, onSelect }) => (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5 md:mb-2">
        <span className="font-semibold text-white text-sm md:text-base">{strategy.name}</span>
        {isSelected && (
      ...
      <p className="text-slate-400 text-xs mb-2 md:mb-3 line-clamp-2">{strategy.description}</p>
      <div className="grid grid-cols-3 gap-1 md:gap-2 text-xs">
        <div className="text-center p-1 md:p-0">
          <div className="text-emerald-400 font-medium">{strategy.settings.winRate}%</div>
          <div className="text-slate-500 text-[10px] md:text-xs">Win Rate</div>
        </div>
        <div className="text-center p-1 md:p-0">
          <div className="text-purple-400 font-medium">1:{strategy.settings.riskRewardRatio}</div>
          <div className="text-slate-500 text-[10px] md:text-xs">R:R</div>
        </div>
        <div className="text-center p-1 md:p-0">
          <div className="text-amber-400 font-medium">{strategy.settings.riskPerTrade}%</div>
          <div className="text-slate-500 text-[10px] md:text-xs">Risk</div>
        </div>
      </div>
    </button>
  );
  ...
  const MetricCard = ({ label, value, subValue, color = 'emerald', tooltipKey, warning = false }) => (
    <div className={`bg-slate-800/50 rounded-lg p-2 md:p-3 border ${warning ? 'border-amber-500/50' : 'border-slate-700/50'}`}>
      <div className="flex items-center justify-between mb-0.5 md:mb-1">
        <span className="text-slate-400 text-[10px] md:text-xs truncate">{label}</span>
        {tooltipKey && <InfoTooltip tooltipKey={tooltipKey} />}
      </div>
      <div className={`text-sm md:text-lg font-bold text-${color}-400`}>{value}</div>
      {subValue && <div className="text-slate-500 text-[10px] md:text-xs truncate">{subValue}</div>}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white p-3 sm:p-4 md:p-6 lg:p-8">
      <style>{`
        ...
        @media (max-width: 640px) {
          .tooltip-container > div:last-child { position: fixed !important; left: 1rem !important; right: 1rem !important; top: auto !important; bottom: 1rem !important; max-height: 60vh; overflow-y: auto; }
        }
      `}</style>
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="text-center mb-6 md:mb-8">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-emerald-400 via-cyan-400 to-purple-400 bg-clip-text text-transparent mb-2">
            Compound Growth Lab
          </h1>
          <p className="text-slate-400 text-sm md:text-base">Trading Portfolio Simulator</p>
        </div>

        {/* Tab Navigation - Scrollable on mobile */}
        <div className="flex gap-2 mb-4 md:mb-6 border-b border-slate-700/50 pb-3 md:pb-4 overflow-x-auto scrollbar-hide">
          {['calculator', 'insights', 'taxCenter'].map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-3 sm:px-4 md:px-6 py-2 rounded-lg font-medium transition-all whitespace-nowrap text-sm md:text-base flex-shrink-0 ${
                activeTab === tab 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
          ...
          <div className="lg:col-span-4 space-y-4 md:space-y-6 order-2 lg:order-1">
            {activeTab === 'calculator' && (
              <>
                ...
                <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 md:p-6">
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                      ...
                      <button
                        ...
                        className={`text-xs px-2 sm:px-3 py-1 rounded-full transition-all ${
                          advancedMode 
                ...
                {advancedMode && (
                  <div className="bg-slate-800/30 backdrop-blur-sm border border-purple-500/30 rounded-xl p-4 md:p-6">
                    <h3 className="text-base md:text-lg font-semibold text-white mb-2 flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-purple-500"></span>
                      Strategy Templates
                    </h3>
                    <p className="text-slate-400 text-xs mb-3 md:mb-4">Select a pre-configured strategy or customize your own parameters below.</p>
                ...
                {advancedMode && (
                  <div className="bg-slate-800/30 backdrop-blur-sm border border-purple-500/30 rounded-xl p-4 md:p-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 md:gap-3">
                      ...
                      <InputField label="Risk/Trade" value={riskPerTrade} onChange={setRiskPerTrade} min={0.1} max={10} step={0.1} suffix="%" tooltipKey="riskPerTrade" small />
                    ...
                    <InputField label="Commission/Trade" value={commissionPerTrade} onChange={setCommissionPerTrade} min={0} max={50} step={0.5} suffix="$" tooltipKey="commissionPerTrade" small />
                    
                    {/* Calculated Metrics */}
                    <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-slate-700">
                      <h4 className="text-sm font-medium text-slate-300 mb-2 md:mb-3">Calculated Metrics</h4>
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <MetricCard 
                          ...
                          subValue={advancedMetrics.winRateVsBreakeven > 0 ? `+${advancedMetrics.winRateVsBreakeven.toFixed(1)}%` : `${advancedMetrics.winRateVsBreakeven.toFixed(1)}%`}
                        ...
                        <MetricCard 
                          ...
                          subValue={`½: ${Math.max(0, advancedMetrics.halfKelly).toFixed(1)}%`}
                          color="purple"
                        ...
                        <MetricCard 
                          ...
                          subValue={advancedMetrics.profitFactor > 1.5 ? 'Good' : advancedMetrics.profitFactor > 1 ? 'Marginal' : 'Loss'}
                        ...
                        <MetricCard 
                          ...
                          subValue={`~${(advancedMetrics.expectedDailyReturn * 252).toFixed(0)}%/yr`}
                          color="cyan"
                        />
                        <MetricCard 
                          label="Fees/Month" 
                          value={`$${advancedMetrics.monthlyCommissions.toFixed(0)}`}
                          subValue={`${advancedMetrics.commissionImpactPercent.toFixed(2)}%/day`}
                          color="amber"
                        />
                      </div>
                      
                      {!advancedMetrics.isPositiveExpectancy && (
                        <div className="mt-2 md:mt-3 p-2 md:p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                          <p className="text-red-400 text-xs font-medium">Warning: Negative Expectancy</p>
                          <p className="text-slate-400 text-xs mt-1">This strategy loses money over time.</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* Monthly Additions - Advanced mode only */}
                {advancedMode && (
                  <div className="bg-slate-800/30 backdrop-blur-sm border border-purple-500/30 rounded-xl p-4 md:p-6">
                    <h3 className="text-base md:text-lg font-semibold text-white mb-3 md:mb-4 flex items-center gap-2">
                    ...
                    <div className="text-xs text-slate-400 mt-2 p-2 bg-slate-900/50 rounded">
                      Fees: ${advancedMetrics.monthlyCommissions.toFixed(0)}/mo ({tradesPerDay} trades × ${commissionPerTrade})
                    </div>
                  </div>
                )}

                {/* Withdrawals - Advanced mode only */}
                {advancedMode && (
                  <div className="bg-slate-800/30 backdrop-blur-sm border border-purple-500/30 rounded-xl p-4 md:p-6">
                    <div className="flex items-center justify-between mb-3 md:mb-4">
                      <h3 className="text-base md:text-lg font-semibold text-white flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-amber-500"></span>
                        <span className="hidden sm:inline">Regular Withdrawals</span>
                        <span className="sm:hidden">Withdrawals</span>
                      </h3>
                      <button
                        onClick={() => setWithdrawalEnabled(!withdrawalEnabled)}
                        className={`w-11 md:w-12 h-6 rounded-full transition-all relative ${withdrawalEnabled ? 'bg-amber-500' : 'bg-slate-700'}`}
                      >
                        <span className={`absolute w-5 h-5 bg-white rounded-full top-0.5 transition-all ${withdrawalEnabled ? 'left-5 md:left-6' : 'left-0.5'}`}></span>
                      </button>
                    </div>
            ...
            {activeTab === 'taxCenter' && (
              <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 md:p-6">
                <h3 className="text-base md:text-lg font-semibold text-white mb-3 md:mb-4">Tax Configuration</h3>
                ...
                <InputField 
                  ...
                  options={Object.entries(STATE_TAX_DATA).map(([code, data]) => ({
                    value: code,
                    label: `${data.name}${!data.hasCapGains ? ' (0%)' : ` (${data.rate}%)`}`
                  }))}
                />
                ...
                <div className="mt-3 md:mt-4 space-y-2 md:space-y-3">
                  <div className="p-2 md:p-3 bg-slate-900/50 rounded-lg">
                    <p className="text-amber-400 text-xs md:text-sm font-medium">Short-Term Capital Gains</p>
                    <p className="text-slate-400 text-[10px] md:text-xs">Day trading profits taxed as ordinary income</p>
                  </div>
                  <div className="p-2 md:p-3 bg-slate-900/50 rounded-lg">
                    <p className="text-emerald-400 text-xs md:text-sm font-medium">Wash Sale Rule</p>
                    <p className="text-slate-400 text-[10px] md:text-xs">Can't claim loss if repurchasing within 61 days</p>
                  </div>
                  <div className="p-2 md:p-3 bg-slate-900/50 rounded-lg">
                    <p className="text-purple-400 text-xs md:text-sm font-medium">Quarterly Estimated Taxes</p>
                    <p className="text-slate-400 text-[10px] md:text-xs">Required if owing $1,000+ annually</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-8 space-y-4 md:space-y-6 order-1 lg:order-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
              <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 border border-emerald-500/30 rounded-xl p-3 md:p-4">
                <p className="text-emerald-400 text-[10px] md:text-xs font-medium mb-0.5 md:mb-1">Final Balance</p>
                <p className="text-lg md:text-2xl font-bold text-white">${summaryStats?.finalBalance?.toLocaleString() || 0}</p>
              </div>
              <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/10 border border-purple-500/30 rounded-xl p-3 md:p-4">
                <p className="text-purple-400 text-[10px] md:text-xs font-medium mb-0.5 md:mb-1">Total Withdrawn</p>
                <p className="text-lg md:text-2xl font-bold text-white">${summaryStats?.totalWithdrawn?.toLocaleString() || 0}</p>
              </div>
              <div className="bg-gradient-to-br from-cyan-500/20 to-cyan-600/10 border border-cyan-500/30 rounded-xl p-3 md:p-4">
                <p className="text-cyan-400 text-[10px] md:text-xs font-medium mb-0.5 md:mb-1">Total Return</p>
                <p className={`text-lg md:text-2xl font-bold ${summaryStats?.totalReturn < 0 ? 'text-red-400' : 'text-white'}`}>{summaryStats?.totalReturn?.toFixed(1) || 0}%</p>
              </div>
              <div className="bg-gradient-to-br from-amber-500/20 to-amber-600/10 border border-amber-500/30 rounded-xl p-3 md:p-4">
                <p className="text-amber-400 text-[10px] md:text-xs font-medium mb-0.5 md:mb-1">Est. Total Tax</p>
                <p className="text-lg md:text-2xl font-bold text-white">${Math.round(summaryStats?.totalTax || 0).toLocaleString()}</p>
              </div>
            </div>
            ...
            {advancedMode && monteCarloResults && (
              <div className="bg-slate-800/30 backdrop-blur-sm border border-purple-500/30 rounded-xl p-4 md:p-6">
                <div className="flex items-center justify-between mb-3 md:mb-4 flex-wrap gap-2">
                  <h3 className="text-sm md:text-lg font-semibold text-white flex items-center gap-1 md:gap-2">
                    <span className="w-2 h-2 rounded-full bg-purple-500"></span>
                    <span className="hidden sm:inline">Monte Carlo Simulation</span>
                    <span className="sm:hidden">Monte Carlo</span>
                    <span className="text-slate-400 text-xs">({simulationRuns})</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 mb-3 md:mb-4">
                  <div className="bg-slate-900/50 rounded-lg p-2 md:p-3 text-center">
                    <div className="text-emerald-400 text-sm md:text-lg font-bold">${monteCarloResults.statistics.median.toLocaleString()}</div>
                    <div className="text-slate-500 text-[10px] md:text-xs">Median</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-2 md:p-3 text-center">
                    <div className="text-cyan-400 text-sm md:text-lg font-bold">{monteCarloResults.statistics.profitablePercent}%</div>
                    <div className="text-slate-500 text-[10px] md:text-xs">Profitable</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-2 md:p-3 text-center">
                    <div className="text-amber-400 text-sm md:text-lg font-bold">{monteCarloResults.statistics.avgDrawdown}%</div>
                    <div className="text-slate-500 text-[10px] md:text-xs">Avg DD</div>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-2 md:p-3 text-center">
                    <div className="text-red-400 text-sm md:text-lg font-bold">{monteCarloResults.statistics.blownUpPercent}%</div>
                    <div className="text-slate-500 text-[10px] md:text-xs">Blown Up</div>
                  </div>
                </div>
                
                <div className="grid grid-cols-3 gap-2 md:gap-3 mb-3 md:mb-4 text-xs md:text-sm">
                  <div className="text-center">
                    <span className="text-slate-400 block md:inline">10th:</span>
                    <span className="text-red-400 md:ml-1">${monteCarloResults.statistics.percentile10.toLocaleString()}</span>
                  </div>
                  <div className="text-center">
                    <span className="text-slate-400 block md:inline">Avg:</span>
                    <span className="text-white md:ml-1">${monteCarloResults.statistics.average.toLocaleString()}</span>
                  </div>
                  <div className="text-center">
                    <span className="text-slate-400 block md:inline">90th:</span>
                    <span className="text-emerald-400 md:ml-1">${monteCarloResults.statistics.percentile90.toLocaleString()}</span>
                  </div>
                </div>
                  ...
                  <div className="h-48 md:h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart>
                        ...
                        <YAxis stroke="#64748b" tick={{ fill: '#94a3b8', fontSize: 10 }} tickFormatter={(v) => `$${(v/1000).toFixed(0)}k`} width={45} />
            ...
            {/* Main Chart */}
            <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 md:p-6">
              <h3 className="text-base md:text-lg font-semibold text-white mb-3 md:mb-4">Portfolio Growth</h3>
              <div className="h-56 md:h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={calculateGrowth}>
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                    <XAxis dataKey="date" stroke="#64748b" tick={{ fill: '#94a3b8', fontSize: 10 }} interval="preserveStartEnd" />
                    <YAxis stroke="#64748b" tick={{ fill: '#94a3b8', fontSize: 10 }} tickFormatter={(v) => `$${(v/1000).toFixed(0)}k`} width={45} />
            ...
            <div className="bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 md:p-6">
              <h3 className="text-base md:text-lg font-semibold text-white mb-3 md:mb-4">Monthly Breakdown</h3>
              
              {/* Negative Trajectory Warning */}
              {summaryStats && summaryStats.totalReturn < 0 && (
                <div className="mb-3 md:mb-4 p-2 md:p-3 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-2 md:gap-3">
                  <svg className="w-4 md:w-5 h-4 md:h-5 text-red-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                  <div>
              ...
              <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
                <table className="w-full text-xs md:text-sm min-w-[500px]">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="text-left py-2 md:py-3 px-1 md:px-2 text-slate-400">Mo</th>
                      <th className="text-right py-2 md:py-3 px-1 md:px-2 text-slate-400">Date</th>
                      <th className="text-right py-2 md:py-3 px-1 md:px-2 text-slate-400">Balance</th>
                      <th className="text-right py-2 md:py-3 px-1 md:px-2 text-slate-400">Growth</th>
                      {withdrawalEnabled && <th className="text-right py-2 md:py-3 px-1 md:px-2 text-slate-400">WD</th>}
                      <th className="text-right py-2 md:py-3 px-1 md:px-2 text-slate-400">Total WD</th>
                    </tr>
                  </thead>
                  <tbody>
                    {calculateGrowth.slice(0, 24).map((row, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-slate-700/50 hover:bg-slate-700/20">
                          <td className="py-2 md:py-3 px-1 md:px-2 text-slate-300">{row.month}</td>
                          <td className="py-2 md:py-3 px-1 md:px-2 text-right text-slate-300 whitespace-nowrap">{row.date}</td>
                          <td className={`py-2 md:py-3 px-1 md:px-2 text-right whitespace-nowrap ${isNegative ? 'text-red-400' : 'text-emerald-400'}`}>${row.balance.toLocaleString()}</td>
                          <td className={`py-2 md:py-3 px-1 md:px-2 text-right whitespace-nowrap ${isNegative ? 'text-red-400' : 'text-cyan-400'}`}>
                            {isNegative ? '' : '+'}{growthPercent.toFixed(1)}%
                          </td>
                          {withdrawalEnabled && (
                            <td className="py-2 md:py-3 px-1 md:px-2 text-right text-amber-400 whitespace-nowrap">
                              {row.monthlyWithdrawal > 0 ? `$${row.monthlyWithdrawal.toLocaleString()}` : '-'}
                            </td>
                          )}
                          <td className="py-2 md:py-3 px-1 md:px-2 text-right text-purple-400 whitespace-nowrap">${row.withdrawn.toLocaleString()}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                {calculateGrowth.length > 24 && (
                  <p className="text-center text-slate-500 py-3 md:py-4 text-xs md:text-sm">Showing first 24 of {calculateGrowth.length} months</p>
                )}
              </div>
            </div>
          </div>
        ...
        {/* Footer */}
        <div className="mt-6 md:mt-8 text-center text-slate-500 text-xs md:text-sm px-4">
